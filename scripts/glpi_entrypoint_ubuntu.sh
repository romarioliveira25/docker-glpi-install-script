...
if [ ! -f /etc/apache2/sites-enabled/glpi.conf ]; then
...
function install_plugin_helper() {
  ...
  case "$FILE" in
    ...
    *.tar.bz2)
      ...
      ;;
    *.zip)
      unzip -o ${FILE} -d ${FOLDER_GLPI_PLUGINS}
      ;;
    *)
  ...
  chown -R www-data:www-data ${FOLDER_GLPI_PLUGINS}/${PLUGIN}
...
function part2() {
  ...
  chown -R www-data:www-data glpi /etc/glpi /var/lib/glpi
